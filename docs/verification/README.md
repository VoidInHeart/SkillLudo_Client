...
`settings-result.json` 及 `settings-*.png` 为桌面 DPR 2 / 手机模拟 DPR 3 的实际指针交互：图鉴位于开房/入房下方、头像可访问、所有场景齿轮、开关点击/拖动、关闭清除浮窗但保留聊天、重新开启不回放、过期、旋转和刷新记忆。棋局为隔离夹具，不是生产网络回合；结果通过且无浏览器错误。
