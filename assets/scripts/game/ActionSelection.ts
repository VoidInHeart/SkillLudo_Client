...
type SelectionState = Pick<GameSnapshot, 'roomId' | 'rollId' | 'currentPlayerId' | 'phase' | 'players' | 'actionOptions' | 'lifecycle'>;
...
export class ActionSelection {
  ...
  public current(state: SelectionState, playerId: string): ActionOption | null {
    ...
    if (key !== this.key || state.lifecycle?.pause || state.phase !== 'WAIT_SELECT_DIE' || state.currentPlayerId !== playerId || state.players.find((p) => p.id === playerId)?.aiControlled) {
  ...
  public choose(state: SelectionState, playerId: string, optionId: string): boolean {
    ...
    if (state.lifecycle?.pause || state.phase !== 'WAIT_SELECT_DIE' || state.currentPlayerId !== playerId || state.players.find((p) => p.id === playerId)?.aiControlled
